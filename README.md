...
![Image generated by this server](docs/assets/demo.png)

*Output of `generate_image` at 1K, 16:9 (10.0 s round trip).*
...
```bash
git clone https://github.com/uehlingeric/banana-mcp.git
cd banana-mcp
uv sync
```
...
```json
{
  "mcpServers": {
    "banana": {
      "command": "uv",
      "args": ["run", "--directory", "/path/to/banana-mcp", "banana-mcp"],
      "env": {
...
Single-module MCP server (`src/banana_mcp/server.py`, ~460 lines) using the Python MCP SDK with stdio transport, packaged with uv and exposed as the `banana-mcp` console script. Gemini API calls run in a thread executor for async compatibility. Settings persist to a local `settings.json` file.
